...
def add_entry( name, type, object_address, dimension_num, line_declaration, line_references):

    symbol_table[name] = { "Type": type, "Object Address": object_address, "Dimension Num": dimension_num, "Line Declaration": line_declaration,"Line References": line_references,}
    ...
    for line in lines:
        ...
        for i, word in enumerate(words):
            if word == "int" or word == "float" or word == "bool" or word == "char":
                ...
                # add row in table
                add_entry(name,type,object_address,dimension_num,line_declaration,line_references)

                typeValue = data_types[word]
                current_address += typeValue

                if (len(words) > i + 2 and words[i + 2].startswith("[") and words[i + 2].endswith("]")):
